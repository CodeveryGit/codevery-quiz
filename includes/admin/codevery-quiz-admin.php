...
if ( !class_exists('Codevery_Quiz_Admin') ) {
    ...
    class Codevery_Quiz_Admin {
        ...
        public function enqueue_styles() {
            ...
            wp_enqueue_style( $this->plugin_name, CODEVERY_QUIZ_PLUGIN_URI_ASSETS . 'css/cquiz-admin.css', array( 'wp-color-picker' ) );
            wp_enqueue_style( 'cquiz-select2', CODEVERY_QUIZ_PLUGIN_URI_ASSETS . 'css/select2.min.css' );
        ...
        public function enqueue_scripts() {
            ...
            wp_enqueue_style('cquiz-select2', CODEVERY_QUIZ_PLUGIN_URI_ASSETS . 'css/select2.min.js', array('jquery') );
            wp_enqueue_script('cquiz-select2', CODEVERY_QUIZ_PLUGIN_URI_ASSETS . 'js/select2.min.js', array('jquery') );
        ...
        public function preview_email() {
            if ( isset( $_GET['preview_quiz_email'] ) ) {
                ...
                if (isset($_GET['quiz_id'])) {
                    $quiz_id = wp_unslash( $_GET['quiz_id'] );
        ...
        public function save_meta_options( $post_id, $post ) {
            ...
            if ( !isset( $_POST['question_metabox_nonce'] )
                || !wp_verify_nonce( $_POST['question_metabox_nonce'], basename( __FILE__ ) ) )
                return $post_id;
            ...
            if ( $post->post_type == CODEVERY_QUIZ_POST_TYPE ) {
                ...
                $quiz_settings = array(
                    'winner_points'             => absint( $_POST['winner_points'] ),
                    'start_button_text'         => sanitize_text_field( $_POST['start_button_text'] ),
                    'expiration_date'           => sanitize_text_field( $_POST['expiration_date'] ),
                    'coupon_amount'             => absint( $_POST['coupon_amount'] ),
                    'exp_date_format'           => sanitize_text_field( $_POST['exp_date_format'] ),
                    'coupon_background_color'   => sanitize_hex_color( $_POST['coupon_background_color'] ),
                    'highlighted_color'         => sanitize_hex_color( $_POST['highlighted_color'] ),
                    'title_quiz_winner'         => sanitize_text_field( $_POST['title_quiz_winner'] ),
                    'text_quiz_winner'          => preg_replace( "/\r|\n/", "", wpautop( $_POST['text_quiz_winner'] ) ),
                    'title_quiz_looser'         => sanitize_text_field($_POST['title_quiz_looser']),
                    'text_quiz_looser'          =>  preg_replace( "/\r|\n/", "", wpautop( $_POST['text_quiz_looser'], false ) ),
                    'hover_answer_color'        => sanitize_hex_color( $_POST['hover_answer_color'] ),
                    'correct_answer_color'      => sanitize_hex_color( $_POST['correct_answer_color'] ),
                    'incorrect_answer_color'    => sanitize_hex_color( $_POST['incorrect_answer_color'] ),
                    'certificate_text'          => sanitize_text_field( $_POST['certificate_text'] ),
                    'promocode_text'            => sanitize_text_field( $_POST['promocode_text'] ),
                    'certificate_sale_text'     => sanitize_text_field( $_POST['certificate_sale_text'] ),
                    'promocode_exp_text'        => sanitize_text_field( $_POST['promocode_exp_text'] ),
                    'coupon_description'        => preg_replace( "/\r|\n/", "", wpautop( $_POST['coupon_description'] ) ),
                    'form_description'          => preg_replace( "/\r|\n/", "", wpautop( $_POST['form_description'] ) ),
                    'display_email_form'        => sanitize_text_field( $_POST['display_email_form'] ),
                );
                $quiz_settings = array_map( function( $value ) {
                    $value = htmlspecialchars( $value, ENT_QUOTES );
                    return $value;
                }, $quiz_settings );
                $quiz_settings_json = json_encode( $quiz_settings, JSON_UNESCAPED_UNICODE );
                if ( $this->json_validator( $quiz_settings_json ) ) {
                    update_post_meta( $post_id, 'quiz_settings', $quiz_settings_json );
                }
                update_post_meta( $post_id, 'email_layout', preg_replace( "/\r|\n/", "", wp_kses_post( $_POST['email_layout'] ) ) );
            }

            if ( $post->post_type == 'quiz_question' ) {
                if ( isset( $_POST['answers'] ) ) {
                    $answers = array_map( array( $this, 'recursive_sanitize_text_field' ), $_POST['answers'] );
                    $answers[$_POST['answer']]['answer'] = true;
                    ...
                    $question_options = json_encode($answers, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES );
        ...
        public function display_meta_quiz_setting( $post_object ) {
            ...
            wp_nonce_field( basename( __FILE__ ), 'question_metabox_nonce' );
        ...
        public function display_shortcode_box( $post_object ) {
            ...
            <div class="codevery-quiz-shortcode">
                ...
                <code id="cquiz_shortcode_<?php echo $post_object->ID; ?>" class="cquiz-blue-bg">[codevery_quiz id=<?php echo $post_object->ID; ?>]</code>
                <a href="#" id="copy_shortcode" class="copy_shortcode" data-copied_text_id="#cquiz_shortcode_<?php echo $post_object->ID; ?>" >
        ...
        public function add_new_question() {
            ...
            $post_data = array(
                'post_title'  => $title,
                'post_content' => trim( $_POST['description'] ),
                'post_status' => 'publish',
                'post_type'   => 'quiz_question',
            );
